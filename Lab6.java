...
public class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        gl2.glMaterialfv(GL2.GL_FRONT,GL_AMBIENT_AND_DIFFUSE,new float[]{1,0.5f,0.8f,1},1);
        Piramida(gl2,15); //Deklaracja wielkości 
    } // end display()
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl2.glLightfv(GL_LIGHT0, GL_POSITION, new float[]{0,3,0,0},0);
        gl2.glLightfv(GL_LIGHT1, GL_POSITION, new float[]{3,0.1f,0,0},0);
        gl2.glLightfv(GL_LIGHT2, GL_POSITION, new float[]{-3,1,-3,0},0);
        koloruj();
        gl2.glLightfv(GL_LIGHT0, GL_SPECULAR,new float[]{q, w, e,1},0);
        koloruj();
        gl2.glLightfv(GL_LIGHT1, GL_DIFFUSE, new float[]{q, w, e,0},0);
        koloruj();
        gl2.glLightfv(GL_LIGHT2, GL_DIFFUSE, new float[]{q, w, e,0},0);
    }
 
    public void Trojkat(GL2 gl2, float x1, float y1, float x2, float y2, float H){
    ...
    public void Piramida(GL2 gl2, int nrOfVer){
        float R = 1;
        int n = nrOfVer;
        ...
        for (int i = 1; i < n; i++) {
            ...
            gl2.glPushMatrix();
            Trojkat(gl2, xPoints[i-1],yPoints[i-1],xPoints[i],yPoints[i],0);
            gl2.glPopMatrix();
            gl2.glPushMatrix();
            Trojkat(gl2, xPoints[i-1],yPoints[i-1],xPoints[i],yPoints[i],2);
            gl2.glPushMatrix();
        }
 
        gl2.glPushMatrix();
        // gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK,GL_EMISSION,new float[]{1,0.4f,0.1f,1},1);
        Trojkat(gl2, xPoints[n-1],yPoints[n-1],xPoints[0],yPoints[0],0);
        gl2.glPopMatrix();
        gl2.glPushMatrix();
        Trojkat(gl2, xPoints[n-1],yPoints[n-1],xPoints[0],yPoints[0],2);
        gl2.glPopMatrix();
        gl2.glPopMatrix();
    }
