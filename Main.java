...
public class Main extends JPanel implements GLEventListener {
	...
	public void pyramid(GL2 gl2, int n) {
		float R = 0.5f;
		int n1 = 15;
		float[] xPoints = new float[n1];
		float[] yPoints = new float[n1];
		xPoints[0] = (float) (R * Math.cos((2.0 * Math.PI * 0) / n1));
		yPoints[0] = (float) (R * Math.sin((2.0 * Math.PI * 0) / n1));


		for (int i = 1; i < n1; i++) {

			xPoints[i] = (float) (R * Math.cos((2.0 * Math.PI * i) / n1));
			yPoints[i] = (float) (R * Math.sin((2.0 * Math.PI * i) / n1));
		...
		Triangle(gl2, xPoints[n1 - 1], yPoints[n1 - 1], xPoints[0], yPoints[0], 0);

		Triangle(gl2, xPoints[n1 - 1], yPoints[n1 - 1], xPoints[0], yPoints[0], 0.5f);
